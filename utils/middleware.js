...
const manejoErrores = (error, request, response, next) => {
  ...
  if (error.name === 'CastError') {
    // Si es una excepción CastError: mal formarto de ID
    return response.status(400).send({ error: 'Formato de ID incorrecto' });
  } else if (error.name === 'ValidationError') {
    // Si es un error en la validación de datos de los campos
    ...
    return response
      .status(400)
      .json({ error: 'se espera que `nombre_usuario` sea único' });
