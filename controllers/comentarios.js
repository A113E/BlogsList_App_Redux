...
comentariosRouter.post('/:blogId', tokenExtractor, usuarioExtractor, async (request, response) => {
    ...
    if (!body.comentario || body.comentario.trim() === '') {
        return response.status(400).json({ error: 'El comentario no puede estar vacio' })
    }
    ...
    await comentarioGuardado.populate('usuario', { nombre_usuario: 1, nombre: 1 })
