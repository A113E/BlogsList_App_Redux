...
blogsRouter.get('/', async (request, response) => {
  ...
  const blogs = await Blog.find({}).populate('usuario', {
    nombre_usuario: 1,
    nombre: 1,
  }); // Populate para mostrar los datos del usuario que creó el blog
  response.json(blogs);
});
...
blogsRouter.delete(
  ...
  async (request, response) => {
    ...
    usuario.blogs = usuario.blogs.filter(
...
blogsRouter.put(
  ...
  async (request, response) => {
    ...
    if (blog.usuario.toString() !== usuario._id.toString()) {
      return response
        .status(403)
        .json({ error: 'Usuario no autorizado para actualizar este blog' });
    }
    ...
    if (blogExistente && blogExistente._id.toString() !== id) {
      return response
        .status(400)
        .json({ error: 'Ya existe un blog con ese título' });
    }
