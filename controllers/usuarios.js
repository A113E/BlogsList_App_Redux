...
usuariosRouter.get('/', async (request, response) => {
  const usuarios = await Usuario.find({}).populate('blogs', {
    ...
    url: 1,
    likes: 1,
  }); // Populate: muestra los blogs creados por el usuario
...
usuariosRouter.post('/', async (request, response) => {
  ...
  if (!nombre_usuario) {
    return response
      .status(400)
      .json({ error: 'Nombre de usuario es requerido' });
  }
  if (!nombre) {
  ...
  if (nombre_usuario.length < 3 || password.length < 3) {
    return response.status(400).json({
      error:
        'Nombre de usuario y contraseña deben de tener al menos 3 caracteres',
    });
  }
