...
describe('Comentarios API', () => {
  ...
  beforeEach(async () => {
    ...
    const loginRespuesta = await api.post('/api/login').send({
      nombre_usuario: 'tester',
      password: 'testing',
    });
  ...
  test('se pueden obtener los comentarios de un blog (aun con array vacio)', async () => {
    ...
    assert.strictEqual(
      ...
      ayuda.comentariosIniciales.length
    );
  });
  ...
  test('un usuario autentificado puede comentar un blog', async () => {
    ...
    assert.strictEqual(
      comentariosAlFinal.length,
      ayuda.comentariosIniciales.length + 1
    );

    // Comprueba que el contenido del comentario existe
    const comentarioContenido = comentariosAlFinal.map(
